...
class MemorySlots(nn.Module):
    ...
    def __init__(self, n_slots: int, mem_dim: int, device: torch.device | None = None):
        ...
        # Learned empty vector (helps training; safe default)
    ...
    def read(self, query: torch.Tensor):
        ...
        # softmax can return NaNs if all logits are -inf (i.e., all slots empty)
        attn = torch.softmax(logits, dim=-1)
        attn = torch.nan_to_num(attn, nan=0.0, posinf=0.0, neginf=0.0)

        read_vec = (attn.unsqueeze(-1) * self.mem).sum(dim=1)  # (B,D)

        # update usage signal (metadata; no grad)
        with torch.no_grad():
    ...
    def step(
        ...
        Masks:
          retain_mask/update_mask/forget_mask: (B, N) bool
          write_mask: (B,) bool
          overwrite_idx: (B,) long (target slot for write)
        ...
        # age increments for alive slots (metadata)
        with torch.no_grad():
        ...
        if bool(update_mask.any()):
            ...
            with torch.no_grad():
                ...
                # Reset age for updated slots (treat as refreshed)
                self._age = torch.where(update_mask, torch.zeros_like(self._age), self._age)

        # RETAIN: keep alive (no change to mem content)
        ...
        # --- IMPORTANT FIX ---
        # Detach memory state to avoid autograd graph being carried across steps/batches.
